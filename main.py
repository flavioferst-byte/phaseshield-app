    ...
    try:
        ...
        if video_has_audio:
            cmd = [
                ...
                "-filter_complex", "[0:a]volume=1.15,pan=mono|c0=0.5*c0+0.5*c1,asetrate=44100*1.012,aresample=44100,atempo=0.988,aformat=sample_fmts=fltp:sample_rates=44100:channel_layouts=stereo,pan=stereo|c0=c0|c1=-1*c0[a0];[1:a]volume=0.01,aformat=sample_fmts=fltp:sample_rates=44100:channel_layouts=stereo[a1];[a0][a1]amix=inputs=2:duration=first:dropout_transition=2:normalize=0[aout]",
                "-map", "0:v:0", "-map", "[aout]",
